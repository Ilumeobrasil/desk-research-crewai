...
# Calcula o caminho raiz do projeto (sobe 4 níveis: utils -> desk_research -> src -> raiz)
...
def run_treatment_folder(
    *,
    ...
    for fp in files:
        ...
        # Obtém UUID do payload ou gera um novo (para compatibilidade com arquivos antigos)
        ...
        # Busca todos os chunks do Asimov para este UUID
        asimov_chunks_text = ""
        ...
        if asimov.enabled and asimov.is_configured() and dataset:
            try:
                chunks_result = asimov.find_snippets(
                    ...
                    max_items=1000,  # Busca até 1000 chunks (suficiente para a maioria dos casos)
                    page_size=30,
                    max_pages=100,
                )
                if chunks_result.get("ok"):
                    ...
                    # Ordena os chunks por key para manter ordem (chunk_01, chunk_02, etc.)
                    sorted_items = sorted(items, key=lambda x: str(x.get("key", "")))
                    # Combina o conteúdo de todos os chunks
                    chunk_contents = []
                    for item in sorted_items:
                        content = item.get("content") or ""
                        if content:
                            chunk_contents.append(str(content).strip())
                    asimov_chunks_text = "\n\n".join(chunk_contents)
        ...
        # Usa os chunks do Asimov se disponíveis, senão usa o texto local
        text_to_use = asimov_chunks_text if asimov_chunks_text else original_text
        ...
        if not cleaned_text:
            ...
            (output_dir / (file_uuid + ".json")).write_text(json.dumps(out_item, ensure_ascii=False, indent=2), encoding="utf-8")
            processed_ok += 1
            continue
        ...
        # grava sempre usando UUID como nome do arquivo
        (output_dir / (file_uuid + ".json")).write_text(json.dumps(out_item, ensure_ascii=False, indent=2), encoding="utf-8")
