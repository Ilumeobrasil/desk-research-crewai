...
"""
Research Tools - Academic Crew
Ferramentas de pesquisa acadÃªmica

ATUALIZADO: Incluindo scrapers customizados ResearchGate e Scielo
"""
...
def serper_scholar_tool(query: str) -> str:
    """
    Busca papers acadÃªmicos no Google Scholar via API Serper
    
    Esta ferramenta DEVE ser usada PRIMEIRO em qualquer pesquisa acadÃªmica.
    Protocolo obrigatÃ³rio: serper_scholar_search â†’ outras ferramentas
    
    Args:
        query: Termo de busca acadÃªmico
    ...
    try:
        ...
        if not api_key:
            return json.dumps({
                "error": "SERPER_API_KEY nÃ£o encontrada no .env",
                "papers": []
            })
        ...
        papers = []
        for idx, result in enumerate(data.get('organic', [])[:20]): # Aumentando para 20
            ...
            paper = {
                ...
                "ano": _extract_year(result.get('publication_info', {}).get('summary', '')),
                "instituicao": None,
                "resumo": result.get('snippet', 'N/A'),
                "citacoes": int(result.get('inline_links', {}).get('cited_by', {}).get('total', 0)),
                "url": link,
                "pdf_url": pdf_link, 
        ...
        return json.dumps({
            "error": f"Erro no Serper Scholar: {str(e)}",
            "papers": []
        })
...
def semantic_scholar_tool(query: str) -> str:
    ...
    try:
            ...
            try:
                ...
                if response.status_code == 429:
                    if attempt < max_retries - 1:
                        ...
                        print(f"⚠️ Semantic Scholar 429 (Rate Limit). Aguardando {wait_time}s...")
        ...
        return json.dumps({
            "error": f"Erro no Semantic Scholar: {str(e)}",
            "papers": []
        })
...
def arxiv_tool(query: str) -> str:
    """
    Busca papers no arXiv (preprints de fÃ­sica, matemÃ¡tica, CS, etc.)
    ...
    try:
        ...
        # Limpar query de caracteres de controle (fix do bug anterior)
        ...
        for idx, result in enumerate(search.results()):
            paper = {
                "titulo": result.title,
                "autores": [author.name for author in result.authors],
                "ano": result.published.year,
                "instituicao": None,  # arXiv nÃ£o fornece instituiÃ§Ã£o
                "resumo": result.summary[:500],
                "citacoes": 0,  # arXiv nÃ£o tem contagem de citaÃ§Ãµes
                "url": result.pdf_url or result.entry_id, # PRIORIDADE TOTAL AO PDF
                "fonte": "arXiv",
        ...
        return json.dumps({
            "error": f"Erro no arXiv: {str(e)}",
            "papers": []
        })


# =============================================================================
# 4. GOOGLE SCHOLAR TOOL (Fallback via scraping)
# =============================================================================
...
def google_scholar_tool(query: str) -> str:
    ...
    try:
        ...
        print("⚠️ Tentando Google Scholar (scholarly)... isso pode demorar ou falhar com CAPTCHA.")
        ...
        try:
             ...
             return json.dumps({"error": "Google Scholar interrompido (Timeout/Captcha)", "papers": []})
        ...
        return json.dumps({
            "error": f"Erro no Google Scholar: {str(e)}",
            "papers": []
        })
...
@tool("pdf_analyzer")
def pdf_analyzer_tool(pdf_url: str) -> str:
    """
    Analisa PDF de paper acadÃªmico
    
    Args:
        pdf_url: URL do PDF
    
    Returns:
        Texto extraÃ­do do PDF ou mensagem de erro
    """
    try:
        import PyPDF2
        ...
        text = ""
        for page in pdf_reader.pages[:10]:  # Primeiras 10 pÃ¡ginas
            text += page.extract_text()
        
        return f"PDF analisado com sucesso. ConteÃºdo extraÃ­do (primeiras 10 pÃ¡ginas):\n\n{text}..."
        
    except requests.HTTPError as e:
        if e.response.status_code == 403:
            return f"âŒ PDF protegido ou atrÃ¡s de paywall: {pdf_url}\nNÃ£o foi possÃ­vel acessar o conteÃºdo."
        return f"âŒ Erro HTTP ao baixar PDF: {str(e)}"
    
    except Exception as e:
        return f"âŒ Erro ao analisar PDF: {str(e)}"


# =============================================================================
# 6. RESEARCHGATE SCRAPER (NOVO) â­
# =============================================================================
...
def researchgate_scraper_tool(query: str, max_results: int = 10) -> str:
    ...
    try:
        ...
        articles = soup.find_all('div', class_='nova-legacy-c-card__body', limit=max_results)
        ...
        return json.dumps({
            "error": f"Erro ResearchGate: {str(e)}",
            "papers": []
        })


# =============================================================================
# 7. SCIELO SCRAPER (NOVO) â­
# =============================================================================

@tool("scielo_scraper")
def scielo_scraper_tool(query: str, max_results: int = 10) -> str:
    """
    Scraper customizado para Scielo
    
    Busca papers no Scielo usando API pÃºblica.
    ...
    try:
        ...
        for idx, doc in enumerate(results[:max_results]):
            paper = {
                "titulo": doc.get('ti_en') or doc.get('ti_pt') or doc.get('ti_es', 'N/A'),
                "autores": doc.get('au', []),
                "ano": int(doc.get('da', '').split('-')[0]) if doc.get('da') else None,
                "instituicao": doc.get('in', [None])[0] if doc.get('in') else None,
                "resumo": (doc.get('ab_en') or doc.get('ab_pt') or doc.get('ab_es', 'N/A'))[:500],
                "citacoes": 0,
    ...
    try:
        ...
        for idx, work in enumerate(results):
            try:
                ...
                if open_access and isinstance(open_access, dict) and open_access.get('is_oa'):
                ...
                paper = {
                    ...
                    "instituicao": (work.get('institutions') or [{}])[0].get('display_name') if work.get('institutions') else None,
                    "resumo": abstract_text,
                    "citacoes": work.get('cited_by_count', 0),
                    "url": pdf_url, # URL principal AGORA É O PDF
                    "pdf_url": pdf_url, 
                    "fonte": "OpenAlex",
                    "revista": (work.get('primary_location') or {}).get('source', {}).get('display_name', 'N/A'),
                    "posicao": idx + 1
                }
                papers.append(paper)
        ...
        return json.dumps({
            "error": f"Erro no OpenAlex: {str(e)}",
            "papers": []
        })
...
# Exportar todas as ferramentas para uso no academic.py
__all__ = [
    'serper_scholar_tool',
    ...
    'google_scholar_tool',
    'pdf_analyzer_tool',
    'researchgate_scraper_tool',  # â­ NOVO
    'scielo_scraper_tool',         # ⭐ NOVO
    'openalex_search_tool',        # ⭐ NOVO
    'url_validator_tool'
]



# ✅ FERRAMENTA DE ANÁLISE DE PDF (versão simplificada)
...
# ✅ FERRAMENTA DE ANÁLISE DE PDF (versão robusta via wrapper)
@tool("pdf_analyzer")
def pdf_analyzer_tool(url: str) -> str:
    ...
    """
    try:
        from desk_research.tools.pdf_analyzer import pdf_analyzer_func
        print(f"🔧 Tool pdf_analyzer chamada com URL: {url}")
        return pdf_analyzer_func(url)
    except Exception as e:
        return f"Erro inesperado no pdf_analyzer: {str(e)}"


# =============================================================================
# 10. GOOGLE SEARCH & WEB SCRAPER (Adicionados para WebCrew)
# =============================================================================
